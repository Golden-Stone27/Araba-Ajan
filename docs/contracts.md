...
- **Python (Adım 5, `racing_rl` 0.6.0):**
  - **Katalog JSON'u:** `python/racing_rl/bridge/track_catalog.json` (şema `race-track-catalog/v1`, paket verisi). Unity'deki `Racing/Tracks/Export Track Catalog (M6)` menüsüyle (`TrackAssets.BuildCatalogJson`, yalnız Editor kodu) üretilir; elle düzenlenmez.
    - İçerik: pist başına `index`, `id`, `asset`, `profile`, `width`, `length_m`, `checkpoints`, `half_width`, `elevation`, `env_config_hash`, `track_hash` (HELLO ile aynı `R` biçimi, Python'da float olarak birebir eşit). `procedural_refs`: `proc:0`, `proc:1`, `proc:7`, `proc:1000` (index −1).
    - EditMode `TrackCatalogExportTests`: `ExportedJson_IsUpToDate` (dosya = üretilen metin; git `autocrlf` yüzünden CRLF normalize edilir) ve `ExportedJson_MatchesFrozenValues` (bu bölümdeki hash'ler). Katalog veya bir pist değişirse menü yeniden çalıştırılır.
  - **`racing_rl.bridge.tracks`:** `load_catalog()`, `resolve_track(name)` → `TrackSpec(id, index, entry)`. Ad kuralları `TrackCatalog.TryResolve` ile aynıdır: kimlik, asset adı veya `proc:<seed>` (1–19 ASCII rakam, işaretsiz, int64). Kimlik kanoniktir (`proc:007` → `proc:7`). Bilinmeyen ad Unity başlatılmadan `UnknownTrackError` (ValueError) verir.
    - Beklenen hash: katalog pisti ve dondurulmuş `proc` tohumları için katalogdaki `env_config_hash`; diğer `proc` tohumları için None (HELLO'dan kaydedilir).
    - `composite_hash({id: hash})`: tek pistte o pistin hash'i; birden çok pistte `multi:` + sıralı ve tekil `id=hash\n` satırlarının SHA-256'sının ilk 16 hex'i (sıra ve tekrar bağımsız).
    - `training_tracks(env_config_hash, extra.tracks)`: checkpoint'in eğitildiği pistleri doğrular (aşağıda).
  - **`UnityVecEnv(track=...)`:** Player'a `-trackName <id>` (yalnız exe ile; `track` ile birlikte `extra_args` içinde `-trackName`/`-trackIndex` verilemez), CONFIG'e `expected_track_id`. HELLO'da `track_id`, `track_index` ve (katalog/dondurulmuş pistlerde) `track_hash` denetlenir; pist alanı olmayan (M6 öncesi) build ile `track` kullanılamaz. `expected_env_hash` varsayılanı `AUTO`: pist yoksa Track_A (M5 ile aynı), pist varsa katalogdaki hash. `env.track_info` = HELLO pist alanları + `env_config_hash` + `profile`. `infos["track_index"]`: ajan başına HELLO `track_index` (salt okunur, önceden ayrılmış dizi; `final_info`'da yok).
    - `track=None` M5 yoludur: bayrak yok, CONFIG'de `expected_track_id` yok, beklenen hash `90240ee2b1a58b5b`.
    - Editor/mock (`exe_path=None`): komut satırı yoktur; pisti sahnedeki `BridgeDriver.trackOverride` seçer, HELLO ve CONFIG denetimi korur.
  - **`MultiUnityVecEnv(tracks=[...])`:** Süreç k, `tracks[k % len]` pistini ömrü boyunca koşturur ve kendi katalog hash'iyle denetlenir. `len(tracks) > K` hatadır; K pist sayısına bölünmüyorsa uyarı verilir. Birden çok pistte açık bir `expected_env_hash` verilemez. `track_ids` (süreç başına), `env_hashes` (`{id: hash}`), `env_config_hash` (= `composite_hash`).
  - **Checkpoint (biçim `racing_rl.ppo/v1` değişmez):** `env_config_hash` tek pistte o pistin hash'i, karışık eğitimde `multi:<sha16>`; `extra.tracks = {id: env_config_hash}` her yeni checkpoint'e yazılır (M5 yolu dahil: `{"Track_A": "90240ee2b1a58b5b"}`). `run.json` aynı iki alanı taşır.
  - **Checkpoint denetimi (C0.11'in M6 hali):** `train.evaluate.load_policy` artık "hash = Track_A" değil, "checkpoint'in eğitim ortamı katalogda" kuralını uygular. `extra.tracks` varsa her katalog/dondurulmuş kimliğin hash'i katalogla aynı ve `composite_hash(extra.tracks)` = `env_config_hash` olmalıdır; yoksa (M4/M5 checkpoint'leri) `env_config_hash` bir katalog hash'i olmalıdır. `obs_layout_hash` birebir aynı olmalıdır. Sim/Vehicle/Reward/gözlem değişikliği tüm katalog hash'lerini değiştirdiği için Env Freeze koruması sürer. Değerlendirilen pist bundan bağımsızdır (sıfır atış). ONNX modelleri hash taşımaz; M2 kaynaklı oldukları için `train_tracks = {"Track_A": …}` yazılır.
  - **CLI:**
    - `train_ppo --track T` (tekrarlanabilir) veya YAML `run.tracks`; doğrulama `run.eval_track` (varsayılan ilk pist) üzerinde, `eval.jsonl` satırlarında `track`. `--resume` beklenen hash'i `run.json`'dan okur ve canlı ortamın bileşik hash'iyle karşılaştırır. Karışık eğitimde model seçimi yalnız ilk pistte yapılır (kullanıcı kararı; çok pistli doğrulama 10M karma eğitimle birlikte sonraya bırakıldı).
    - `train.evaluate --track T` (`--exe ""` Editor/mock). Rapora `track` (HELLO alanları) ve `train_tracks` eklenir; `env_config_hash` değerlendirilen pistindir, bu yüzden `compare.py` farklı pistlerin raporlarını eşleştirmez. `per_seed` anahtarları M5 ile aynıdır.
    - `train.watch --track T`: Editor'de `trackOverride` ile aynı kimlik yazılır; başka pist `ProtocolMismatchError` ile reddedilir.
  - **Testler:** `tests/test_m6_tracks.py` (mock, 41): katalog, ad çözümleme, `composite_hash`, `training_tracks`, HELLO/CONFIG denetimleri ve uyuşmazlıklar, M6 öncesi build, `proc` tohumları, çoklu pist dağılımı, `train_ppo --track` (tek ve karışık, `--resume`), YAML `run.tracks`, `evaluate --track` raporu, `load_policy` kabul/ret. `tests/test_unity_tracks.py` (`-m unity`, 8): A–D, `proc:7`, `proc:42` duman testleri (HELLO alanları katalogla birebir, EvalGrid'den 50 adım), player'dan `UNKNOWN_TRACK`, iki pistli `MultiUnityVecEnv`.
  - **Regresyon:** Python katmanı değiştikten sonra `m6_bridge_check.py regress` yeniden koşuldu: 13/13 koşu metrik ve iz düzeyinde M5 ile aynı (`runs/m6/step5_track_a_regression.json`, gitignore). `m6_zero_shot.py` Track_A'yı yeni API'yle (`track="Track_A"`) koşar ve 6 modelin per-seed metriklerinin M5 ile birebir aynı olduğunu denetler.
  - **PurePursuit referansları (PlayMode, grid, tohum 1000):** `proc:0` 57.72 / 55.88 / 55.90 s, `proc:1` 59.04 / 56.22 / 56.24 s, `proc:7` 47.62 / 45.16 / 45.16 s, `proc:1000` 47.00 / 44.44 / 44.44 s (PlayMode'a `proc:1` ve `proc:1000` eklendi).
- **Sıfır atış (Adım 5, `benchmarks/M6_TRACKS.md`):** M5 modelleri (özel PPO `best.pt` s1–s3, ML-Agents ONNX s1–s3; yalnız Track_A'da eğitildi) C0.10 ile Track_B, C, D ve `proc:0/1/7/1000`'de değerlendirildi (`python/scripts/m6_zero_shot.py run|report`, ham sonuçlar `benchmarks/eval/m6_zero_shot.json`, izler `runs/m6/zero_shot/traces`, gitignore).
  - Aynı betik Track_A'yı yeni API'yle (`track="Track_A"`, pist başına tek süreç, modeller arasında RESET) koşar; 6/6 modelin per-seed metrikleri M5 ile birebir aynıdır.
